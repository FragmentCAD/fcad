        ...
        match event {
            InputEvent::Click { button, x, y } if *button == MouseButton::Left => {
                if let Some(p1) = self.first_point {
                    let result = ToolResult::Rectangle {
                        p1,
                        p2: [*x, *y],
                    };
                    self.first_point = None;
...
mod tests {
    ...
    fn test_rect_tool_flow() {
        ...
        // 1. First click
        let resp1 = tool.on_input(&InputEvent::Click {
        ...
        let resp2 = tool.on_input(&InputEvent::PointerMove {
            x: 10.0,
            y: 5.0,
        }, &si);
        ...
        // 3. Second click
        let resp3 = tool.on_input(&InputEvent::Click {
            button: MouseButton::Left,
            x: 10.0,
            y: 5.0,
        }, &si);
        assert_eq!(resp3, ToolResponse::Completed(ToolResult::Rectangle {
            p1: [0.0, 0.0],
            p2: [10.0, 5.0],
        }));
    }

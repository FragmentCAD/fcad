...
mod tests {
    ...
    fn test_fts5_semantic_search() {
        let mut rag = RagEngine::new_in_memory().expect("No se logró inicializar RAG engine en SQLite");
        
        // Cargar un YAML/Estándar falso y registrarlo
        let wall_standard = LayerStandard {
            name: "A-WALL".to_string(),
            description: "Paredes de Construcción Arquitectónica, Estructuras y División, Muros, Tabiques".to_string(),
        ...
        rag.index_standard(&wall_standard).expect("Falló la Indexación de muro");
        rag.index_standard(&door_standard).expect("Falló la Indexación de puerta");
        ...
        assert_eq!(results.len(), 1, "Solo 'A-WALL' debe devolver resultados de muros");

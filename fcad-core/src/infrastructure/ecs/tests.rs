...
mod tests {
    ...
    fn test_ecs_insert_and_query_layer() {
        ...
        for i in 0..1000 {
            ...
            let layer_name = if i % 2 == 0 {
                "A-WALL".to_string()
            } else {
                "A-DOOR".to_string()
            };

            world.spawn((Geometry::Line(line), Layer(layer_name)));
        }
        ...
        assert_eq!(
            a_wall_count, 500,
            "Should exactly retrieve 500 A-WALL entities from memory"
        );
    }
    ...
    fn test_tombstoning_deletion() {
        ...
        let active_count = world
            .query_filtered::<&Layer, Without<Deleted>>()
            .iter(&world)
            .count();
        assert_eq!(
            active_count, 0,
            "Deleted entities should be filtered out from active queries"
        );
    }
    ...
    fn test_spatial_index_reactivity() {
        ...
        let intersections: Vec<_> = spatial_index
            .tree
        ...
        assert_eq!(
            intersections.len(),
            1,
            "Should only find the line intersecting the box"
        );
        assert_eq!(
            intersections[0].id, line_id,
            "The intersecting entity must be the line"
        );
        ...
        let intersections_after_delete: Vec<_> = spatial_index_after_delete
            .tree
            .locate_in_envelope_intersecting(&search_envelope)
            .collect();

        assert_eq!(
            intersections_after_delete.len(),
            0,
            "Deleted entity should have been removed from the R-Tree"
        );
    }

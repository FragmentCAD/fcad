use crate::domain::math::primitives::{Arc as DxfArc, Circle, Line, Point2D};
...
fn parse_entity(
    ...
    match &entity.specific {
        ...
        EntityType::Arc(a) => {
            ...
            world.spawn((
                Geometry::Arc(DxfArc::new(center, a.radius, start_angle, end_angle)),
                layer,
            ));
        }
...
mod tests {
    ...
    fn test_dxf_fuzzing_filter() {
        ...
        assert_eq!(
            geoms_count, 1,
            "Debería haber ignorado la malla 3D y rescatado únicamente la línea 2D"
        );
    ...
    fn test_dxf_bidirectionality_export() {
        ...
        world1.spawn((
            Geometry::Line(Line::new(Point2D::new(0.0, 0.0), Point2D::new(5.0, 5.0))),
            Layer("A-WALL".to_string()),
        ));
        world1.spawn((
            Geometry::Circle(Circle::new(Point2D::new(10.0, 10.0), 3.0)),
            Layer("A-DOOR".to_string()),
        ));

        // Añadimos otro que luego es "Deleted" (y que debe ser ignorado en e port)
        let deleted_id = world1
            .spawn((
                Geometry::Line(Line::new(Point2D::new(0.0, 0.0), Point2D::new(1.0, 1.0))),
                Layer("A-TRASH".to_string()),
            ))
            .id();
        ...
        let count_active_w1 = world1
            .query_filtered::<&Geometry, Without<Deleted>>()
            .iter(&world1)
            .count();
        let count_active_w2 = world2
            .query_filtered::<&Geometry, Without<Deleted>>()
            .iter(&world2)
            .count();
        ...
        assert_eq!(
            count_active_w1, 2,
            "El mundo 1 debía tener 2 componentes activos"
        );
        assert_eq!(
            count_active_w2, 2,
            "El mundo 2 debía reconstruir exactamente los 2 mismos componentes"
        );
        assert_eq!(
            count_active_w1, count_active_w2,
            "La asimetría de componentes quebró la presunción de bidireccionalidad"
        );
